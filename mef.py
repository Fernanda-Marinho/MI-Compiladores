...
def analisar_lexico():
    ...
    for file_path in (os.listdir(current)):
        ...
        if (file_path.endswith('-lexico.txt') or not file_path.endswith(".txt") or file_path.endswith('-saida.txt')): 
            continue
        file = open(f'{current}/{file_path}', 'r')
        newfile = open(f'{current}/{os.path.splitext(os.path.basename(file.name))[0]}-lexico.txt', 'w')
